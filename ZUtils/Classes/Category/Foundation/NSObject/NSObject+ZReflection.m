...
@implementation NSObject (ZReflection)
...
{
    return [[self class] z_propertyKeys];
}
+ (NSArray *)z_propertyKeys {
...
{
    return [[self class] z_propertiesInfo];
}
/**
...
+ (NSArray *)z_propertiesInfo
{
    ...
    for (int i = 0; i < propertyCount; i++)
    {
        [propertieArray addObject:({
            ...
            NSDictionary *dictionary = [self z_dictionaryWithProperty:properties[i]];
...
+ (NSArray *)z_propertiesWithCodeFormat
{
    ...
    NSArray *properties = [[self class] z_propertiesInfo];
...
-(NSArray*)z_methodListInfo{
    ...
    for (int i = 0; i < count ; i++)
    {
        ...
        for (int index=0; index<argumentsCount; index++) {
            ...
            [arguments addObject:[[self class] z_decodeType:arg]];
        }
        
        NSString *returnTypeString =[[self class] z_decodeType:returnType];
        NSString *encodeString = [[self class] z_decodeType:encoding];
...
-(NSDictionary *)z_protocolList{
    return [[self class]z_protocolList];
}
+ (NSDictionary *)z_protocolList
{
...
+ (NSArray *)z_instanceVariable
{
    ...
    NSMutableArray *result = [NSMutableArray array];
    for (int i = 0; i < outCount; i++) {
        NSString *type = [[self class] z_decodeType:ivar_getTypeEncoding(ivars[i])];
...
#pragma mark -- help
+ (NSDictionary *)z_dictionaryWithProperty:(objc_property_t)property
{
...
+ (NSString *)z_decodeType:(const char *)cString
{
    ...
    } else
    {
        if ([[result substringToIndex:1] isEqualToString:@"^"]) {
            result = [NSString stringWithFormat:@"%@ *",
                      [NSString z_decodeType:[[result substringFromIndex:1] cStringUsingEncoding:NSUTF8StringEncoding]]];
        }
    }
    return result;
}
