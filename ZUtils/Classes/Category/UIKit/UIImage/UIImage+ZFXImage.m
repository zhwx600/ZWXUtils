...
@implementation UIImage (ZFXImage)
...
- (UIImage *)z_imageScaledToFitSize:(CGSize)size
{
    ...
    {
        return [self z_imageScaledToSize:CGSizeMake(size.width, size.width / aspect)];
    }
    else
    {
        return [self z_imageScaledToSize:CGSizeMake(size.height * aspect, size.height)];
    }
}
...
- (UIImage *)z_reflectedImageWithScale:(CGFloat)scale
{
    ...
    //clip to gradient
    CGContextClipToMask(context, bounds, [[self class] z_gradientMask]);
...
{
    //get reflected image
    UIImage *reflection = [self z_reflectedImageWithScale:scale];
