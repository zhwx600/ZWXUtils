...
@implementation UIImage (ZBetterFace)

- (UIImage *)z_betterFaceImageForSize:(CGSize)size
                              accuracy:(ZAccuracy)accurary;
{
    NSArray *features = [UIImage _z_faceFeaturesInImage:self accuracy:accurary];
    ...
    if ([features count]==0) {
        ...
        return [self _z_subImageForFaceFeatures:features
                                            size:size];
    }
}

- (UIImage *)_z_subImageForFaceFeatures:(NSArray *)faceFeatures size:(CGSize)size
{
...
+ (NSArray *)_z_faceFeaturesInImage:(UIImage *)image accuracy:(ZAccuracy)accurary
{
    CIImage *ciImage = [CIImage imageWithCGImage:image.CGImage];
    NSString *accuraryStr = (accurary == ZAccuracyLow) ? CIDetectorAccuracyLow : CIDetectorAccuracyHigh;
