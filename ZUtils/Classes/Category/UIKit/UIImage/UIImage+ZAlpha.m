...
@interface UIImage (ZAlphaPrivateMethods)
- (CGImageRef)z_newBorderMask:(NSUInteger)borderSize size:(CGSize)size;
@end

@implementation UIImage (ZAlpha)
/**
 *  @brief  是否有alpha通道
 *
 *  @return 是否有alpha通道
...
- (UIImage *)z_transparentBorderImage:(NSUInteger)borderSize {
    ...
    UIImage *image = [self z_imageWithAlpha];
    ...
    CGImageRef maskImageRef = [self z_newBorderMask:borderSize size:newRect.size];
