...
@implementation UIDevice (ZHardware)
+ (NSString *)z_platform{
    size_t size;
...
+ (NSString *)z_platformString{
    NSString *platform = [self z_platform];
...
+ (NSUInteger)z_getSysInfo:(uint)typeSpecifier
{
...
+ (NSUInteger)z_cpuFrequency {
    return [self z_getSysInfo:HW_CPU_FREQ];
}

+ (NSUInteger)z_busFrequency {
    return [self z_getSysInfo:HW_BUS_FREQ];
}

+ (NSUInteger)z_ramSize {
    return [self z_getSysInfo:HW_MEMSIZE];
}

+ (NSUInteger)z_cpuNumber {
    return [self z_getSysInfo:HW_NCPU];
}


+ (NSUInteger)z_totalMemoryBytes
{
    return [self z_getSysInfo:HW_PHYSMEM];
}
