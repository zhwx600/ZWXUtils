...
@implementation UINavigationItem (ZMargin)
...
- (UIBarButtonItem *)spacerForItem:(UIBarButtonItem *)item withMargin:(CGFloat)margin
{
    ...
    spacer.width = margin - [self.class z_systemMargin];
    if (!item.customView) {
...
{
    return [self spacerForItem:item withMargin:self.z_leftMargin];
}

- (UIBarButtonItem *)rightSpacerForItem:(UIBarButtonItem *)item
{
    return [self spacerForItem:item withMargin:self.z_rightMargin];
}


#pragma mark - Margin

- (CGFloat)z_leftMargin
{
    NSNumber *value = objc_getAssociatedObject(self, @selector(z_leftMargin));
    return value ? value.floatValue : [self.class z_systemMargin];
}

- (void)setZ_leftMargin:(CGFloat)leftMargin
{
    objc_setAssociatedObject(self, @selector(z_leftMargin), @(leftMargin), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    self.leftBarButtonItems = self.leftBarButtonItems;
}

- (CGFloat)z_rightMargin
{
    NSNumber *value = objc_getAssociatedObject(self, @selector(z_rightMargin));
    return value ? value.floatValue : [self.class z_systemMargin];
}

- (void)setZ_rightMargin:(CGFloat)rightMargin
{
    objc_setAssociatedObject(self, @selector(z_rightMargin), @(rightMargin), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
