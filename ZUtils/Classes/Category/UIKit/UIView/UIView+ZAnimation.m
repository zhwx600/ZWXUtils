...
float z_radiansForDegrees(int degrees) {
...
- (void)z_moveTo:(CGPoint)destination duration:(float)secs option:(UIViewAnimationOptions)option {
    [self z_moveTo:destination duration:secs option:option delegate:nil callback:nil];
}
...
- (void)z_raceTo:(CGPoint)destination withSnapBack:(BOOL)withSnapBack {
    [self z_raceTo:destination withSnapBack:withSnapBack delegate:nil callback:nil];
}
...
- (void)z_rotate:(int)degrees secs:(float)secs delegate:(id)delegate callback:(SEL)method {
    [UIView animateWithDuration:secs
                        ...
                        options:UIViewAnimationOptionCurveLinear
                     animations:^{
                         self.transform = CGAffineTransformRotate(self.transform, z_radiansForDegrees(degrees));
                     }
                     completion:^(BOOL finished) {
                         if (delegate != nil) {
#pragma clang diagnostic push
...
- (void)z_spinClockwise:(float)secs {
    [UIView animateWithDuration:secs/4
                        ...
                        options:UIViewAnimationOptionCurveLinear
                     animations:^{
                         self.transform = CGAffineTransformRotate(self.transform, z_radiansForDegrees(90));
                     }
                     completion:^(BOOL finished) {
                         [self z_spinClockwise:secs];
                     }];
}
...
- (void)z_spinCounterClockwise:(float)secs {
    [UIView animateWithDuration:secs/4
                        ...
                        options:UIViewAnimationOptionCurveLinear
                     animations:^{
                         self.transform = CGAffineTransformRotate(self.transform, z_radiansForDegrees(270));
                     }
                     completion:^(BOOL finished) {
                         [self z_spinCounterClockwise:secs];
                     }];
}
...
- (void)z_drainAway:(float)secs {
    self.tag = 20;
    [NSTimer scheduledTimerWithTimeInterval:secs/50 target:self selector:@selector(z_drainTimer:) userInfo:nil repeats:YES];
}

- (void)z_drainTimer:(NSTimer*)timer {
    ...
    [UIView animateWithDuration:secs/2
                     ...
                     completion:^(BOOL finished) {
                         [UIView animateWithDuration:secs/2
                                          ...
                                          completion:^(BOOL finished) {
                                              if (continuously) {
                                                  [self z_pulse:secs continuously:continuously];
                                              }
                                          }];
                     }];
}
